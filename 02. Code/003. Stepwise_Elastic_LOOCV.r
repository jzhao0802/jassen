...
LOOCV_stepwise_v1 <- function(cohort, outcome, j){
    ...
    covar_definition_matched <- covar_definition[match(rownames(coef), covar_definition[, 2]), 4] 
    ...
    #auc_esti <- roc$auc 
    auc_esti <- auc(training_data$response, test_pred_list)
    ...
    write.xlsx(auc_output, file=paste('LOOCV_AUC_135_v1.xlsx', sep=''), sheetName=paste('stepwise_', j, sep=''), row.names=F, append=T, showNA=T)
...
output1 <- LOOCV_stepwise_v1('mild_uni','ga_outcome', 1)
#output2 <- LOOCV_stepwise_v1('mild_uni','adv_outcome')
output3 <- LOOCV_stepwise_v1('mild_bil','ga_outcome', 3)
#output4 <- LOOCV_stepwise_v1('mild_bil','adv_outcome')
output5 <- LOOCV_stepwise_v1('ga_all','wet_outcome', 5)
#output <- rbind(output1, output2, output3, output4, output5)
output <- rbind(output1, output3,  output5)
write.xlsx(auc_output, file=paste('LOOCV_AUC_135_v1.xlsx', sep=''), sheetName='stepwise_5', row.names=F, append=T, showNA=T)
...
Model_Evaluate_elastic_v2 <- function(cohort, outcome){
    ...
    alpha_levels <- length(levels(as.factor(model_select1$alpha)))
    model_select2 <- model_select1[model_select1$auc_diff==min(model_select1$auc_diff), ]
    ...
    write.xlsx(model_select, file=paste('loocv_elastic.xlsx', sep=''), sheetName=paste(cohort, '_by_', outcome, sep=''), row.names=F, append=T, showNA=T)
